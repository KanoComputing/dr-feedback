...
# suggesting that a major problem might have raised, and detailed look should
# be addressed.
#
# The functionality is divided in two steps:
#
#  1) Collect and save a historic log with number of alerts received to the
#     server
#  2) Read and process this log to detect any spike overtime, send and alert in
#     such cases
#
...
"""
DRFeedback-monitor keeps track and alerts the Kano Team about potential Kano OS
customer troubles
...
Options:
   ...
   -d, --directory=<pathname>   Directory to check for numbers of files and
                                collect counter
   -s, --samples=<n>            Number of samples to group for computing
                                average
   -t, --threshold=<factor>     Threshold multitplier for the last average, if
                                above then spike is raised
...
class Collect(object):
    '''
    Collects the current number of Dr Feedback alerts and saves it into a
    logfile
    '''
    def __init__(self, args):
        self.args = args
        self.collect_command = 'ls -l {} | wc -l'.format(
            self.args['--directory']
        )
    ...
    def _collect_counter_(self):
        ...
        return 'date={} total-logs={}'.format(
            strftime("%d-%m-%Y", gmtime()), log_count
        )
    ...
    def run(self):
        ...
        if self.args['--commit']:
            ...
            self.vprint(
                'Commited counter to logfile: {}'
                .format(self.args['--logfile'])
            )
...
class Process(object):
    ...
    def _get_log_counters_(self):
        ...
        with open(self.args['--logfile'], 'r') as log_f:
            logdata = log_f.readlines()

        for log in logdata:
            counters.append(int(log.split('=')[2]))
    ...
    def _compute_(self, numbers):
        '''
        Reads a list of counters obtained from the logfile, and detects a
        spike, given the number of samples to average, and a threshold to
        compute the maximum deviation allowed
        ...
        for index, counter in enumerate(reversed(numbers)):
            ...
            if (index + 1) % self.args['--samples'] == 0:
                ...
                self.vprint(
                    'Average number of drfeedback alerts is {} over last {} '
                    'samples'.format(average_alerts, self.args['--samples']))
                    ...
                    if self._detect_spike(average_alerts, last_average_value):
                        self.vprint(
                            ...
                            'threshold={} max-permitted-average={}'.format(
                                ...
                                last_average_value,
                                self.args['--threshold'],
                                average_alerts * self.args['--threshold']
                            )
                        )
    ...
    def _detect_spike(self, average_alerts, last_average):
        ...
        '''
        self.vprint(
            'Spike check: current-average={} last-average={} threshold={} '
            'max-permitted-average={}'.format(
                average_alerts,
                last_average,
                self.args['--threshold'],
                average_alerts * self.args['--threshold']
        ...
        if is_spike:
            self.vprint(
                'Spike was detected! {} * {} < {}'
                .format(
                    average_alerts, self.args['--threshold'], last_average
                )
            )
    ...
    def slack_alert(self, simulate):
        '''
        Sends a network slack alert to the Dev Team to bring attention to
        DrFeedback logs
        '''

        url = self.url_simulate if simulate else self.url_slack
        self.vprint(
            'Slack alert signal requested, simulate={} url={}'
            .format(simulate, url)
        )
